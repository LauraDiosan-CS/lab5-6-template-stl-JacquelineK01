...
Produs::~Produs()
{
	...
	this->pret = 0; //am adaugat
	this->nr_exemplare = 0; //am adaugat
...
void Produs::setNume(const char* nume) //am adaugat
{
	if (this->nume != NULL)
	{
		delete[] this->nume;
...
void Produs::setPret(int pret) //am adaugat
{
	this->pret = pret;
}

void Produs::setNrExemplare(int nr) //am adaugat
{
	nr_exemplare = nr;
}
...
Produs& Produs::operator=(const Produs& p)
{
	...
	//poate trebuia doar this->setNume(p.nume);
	this->id = p.id;  //this->setPret(p.pret);
	this->pret = p.pret;  //this->setPret(p.pret);
	this->nr_exemplare = p.nr_exemplare;
	return*this;
}
